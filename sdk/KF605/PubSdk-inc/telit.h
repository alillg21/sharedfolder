...
//#define CUR_PDP_CID	1  	//当前使用的PDP上下文ID
#define GPRS_TCP	0
...
//功    能:       Set command selects the flow control behaviour of the serial port in both
...
//函 数 名:       GPRS_SocketConfigExt
//功    能:       opens a remote connection via socket.
//输入参数:
//              connId:         socket connection identifier, 1..6
//              srMode:   SRing unsolicited mode 0-Normal;1-data length;2-Data view;
//      recvDataMode : data view mode for received data in command mode
...
//                                              255 - local host closes only after an escape sequence (+++)
//                      lPort:          UDP connections local port, 0..65535
//      mode :   connection mode ; 0 :online mode connection(default);1: command mode connection;
//输出参数: respone:        设备返回的应答
//返 回 值:
//                      GPRS_SUCCESS:   执行成功
//                      GPRS_RS_TIMEOUT:接收应答超时
...
//功    能:发送数据(必须在socket连接以后才能调用该函数)
//输入参数:
//      socket_fd: Socket connection identifier
//                      data:要发送的数据
//                      data_len:要发送的数据的长度
//                      timeout:该命令的响应超时时间
//返 回 值:
