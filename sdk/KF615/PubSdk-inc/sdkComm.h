...
typedef struct
{
    ...
    u32     uiPort;                 //服务器端口号
    u8      szSecondServerIP[64];   //备用服务器 IP 地址
    u32     uiSecondPort;           //备用服务器端口号
...
typedef s32 (*CommCallback) (void *pParam);

s32 sdkCommInit(const T_SDK_COMM_COMMPARAM *pstCommParam);
s32 sdkCommPre(void);
//
s32 sdkCommLink(void);
s32 sdkCommConnect(u8 *pucRemoteIP, u32 uiRemotePort);
s32 sdkCommDelLink(void);
s32 sdkCommSendData(const u8 *phexData, u32 uiDataLen);
s32 sdkCommRecvData(u8 *phexData, u32 uiDataLen);
//
s32 sdkCommUartGetNum(void);
void sdkCommCallbackSimPIN(CommCallback pfnEnterPIN);
void sdkCommCallbackIMSI(CommCallback pfnParseIMSI);
void sdkCommCallbackIMEI(CommCallback pfnParseIMEI);
